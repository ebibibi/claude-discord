...
- **会話の巻き戻し** — `/rewind` で Claude が作成した作業ファイルを保持しつつ会話履歴のみをリセット。確認メッセージにリセット時点のコンテキスト使用率が表示されるため、なぜ巻き戻したかを把握しやすい
- **会話のフォーク** — `/fork` で現在のスレッドから同じセッション状態を引き継ぐ新しいスレッドを作成（`--fork-session` を使用）。元のスレッドに影響を与えず、別の方向を探索できる
- **コンテキスト可視化** — `/context` でコンテキストウィンドウの使用率をビジュアルプログレスバーで表示。83.5% の閾値に近づくと ⚠️ 自動コンパクト警告を表示
- **レートリミット監視** — `/usage` で Claude API のレートリミット使用率をパーセンテージバーとリセットまでの残り時間付きで表示
